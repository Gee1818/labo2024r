...
PARAM$semillas <- c(276277, 311609, 520067, 535571, 696343)
...
ArbolEstimarGanancia <- function(semilla, param_basicos) {
  ...
  ganancia_test <- dataset[fold == 2, sum(ifelse(
    prediccion[, "BAJA+2"] > 0.025,
...
ArbolesMontecarlo <- function(semillas, param_basicos) {
  ...
  ganancias <- mcmapply(
    ...
    semillas,
    # paso el vector de semillas
    MoreArgs = list(param_basicos),
    # aqui paso el segundo parametro
    SIMPLIFY = FALSE,
...
for (vmax_depth in c(4, 6, 7, 8, 10)) {
  for (vmin_split in c(1000, 700, 600, 500, 400, 100, 70)) {
    for (vmin_bucket in as.integer(c(vmin_split / 2, vmin_split / 3, vmin_split /
                                     4, vmin_split / 5))) {
      ...
      param_basicos <- list(
        ...
        # minima cantidad de registros en una hoja
        "maxdepth" = vmax_depth
